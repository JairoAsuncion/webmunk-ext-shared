...
export class FirebaseAppService {
  ...
  // Route callable functions to europe-west4 (closest to NL)
  private readonly functions = getFunctions(this.firebaseApp, "europe-west4");
  ...
  public async getUser(): Promise<User> {
    ...
    const interval = typeof USER_FETCH_INTERVAL === 'string'
      ? parseInt(USER_FETCH_INTERVAL, 10) || 0
      : (USER_FETCH_INTERVAL ?? 0);

    if (this.userFetchTimestamp && interval > 0 && (Date.now() - this.userFetchTimestamp) < interval) {
      return this.user;
    }
